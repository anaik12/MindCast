...
public class GazeGestureManager : MonoBehaviour
{
	...
	IEnumerator<object> PostToFaceAPI(byte[] imageData, Matrix4x4 cameraToWorldMatrix, Matrix4x4 pixelToCameraMatrix) {
		...
		foreach (var result in j.list) {
            ...
            //txtMesh.text = string.Format("\nMsg: {7}", "You look Fantastic!!!");
            txtMesh.text = string.Format("Gender: {0}\nAge: {1}\nMoustache: {2}\nBeard: {3}\nSideburns: {4}\nGlasses: {5}\nSmile: {6}\nYaw: {7}\nMessage: {8}", a.GetField("gender").str, a.GetField("age"), f.GetField("moustache"), f.GetField("beard"), f.GetField("sideburns"), a.GetField("glasses").str, a.GetField("smile"), h.GetField("yaw"), "You look fantastic");
            //txtMesh.text = string.Format("\nMsg: {7}", "You look Fantastic!!!");
		}
